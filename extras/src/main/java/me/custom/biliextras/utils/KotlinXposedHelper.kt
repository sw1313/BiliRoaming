...
@PublishedApi internal inline fun createHooker(crossinline callback: HookCallback) = object : XposedInterface.Hooker {
    override fun intercept(chain: XposedInterface.Chain): Any? = try {
        ...
        // Coroutine cancellation (e.g. kotlinx ChildCancelledException) is structured-
        // concurrency control flow, not an error. It must propagate to the native
        // coroutine machinery; swallowing it and re-proceeding would re-run cancelled
        // work (this broke native background auto-play of related videos).
        throw e
    } catch (e: Throwable) {
...
fun Class<*>.hookAllMethods(methods: Iterable<Method>, hooker: XposedInterface.Hooker): Set<HookHandle> =
    methods.mapNotNull { method ->
        method.isAccessible = true
        hookExecutable(method, hooker)
    }.toSet()

inline fun Class<*>.hookAllMethods(methods: Iterable<Method>, crossinline callback: HookCallback) =
    hookAllMethods(methods, createHooker(callback))
